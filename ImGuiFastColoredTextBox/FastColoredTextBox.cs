...
namespace FastColoredTextBoxNS
{
    ...
    /// </summary>
    public partial class FastColoredTextBox : ISupportInitialize
    {
        ...
        private readonly System.Timers.Timer timer3 = new System.Timers.Timer();
...
^\s*(case|default)\s*[^:]*(?<range>:)\s*(?<range>[^;]+);";
            ...
            for (int i = 0; i < line.Count - 1; i++)
            {
                ...
                else
                {
                    ...
                    if (!char.IsLetterOrDigit(c) && c != '_' && c != '\'' && c != '\xa0' 
                        && ((c != '.' && c != ',') || !char.IsDigit(line[i + 1]))) //dot before digit
        ...
        protected bool ProcessKeyMessage(ref Message m)
        {
            if (m.Msg == WM_CHAR)
